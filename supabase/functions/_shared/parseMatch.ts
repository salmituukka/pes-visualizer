...
export function parseMatch(
  matchId: number,
  rawResponse: RawEventsResponse | null
): ParsedMatch {
...
function computeAtBatAggregates(
  atbat: ParsedAtBat,
  has_pitch_detail: boolean
): void {
  ...
  // Alkuperäinen kärki
  atbat.original_lead_player = findLeadRunner(
    atbat.effective_start_bases,
    atbat.batter_id
  );
  ...
  if (atbat.original_lead_player !== null) {
    atbat.original_lead_disqualified = all_segments.some(
      s => s.player_id === atbat.original_lead_player &&
           (s.reason === 'hit_out' || s.reason === 'non_pitch_out')
    );
  }
  
  // any_out
  atbat.any_out = all_segments.some(
    s => s.reason === 'hit_out' || s.reason === 'non_pitch_out'
  );
 ...
 */
export function findLeadRunner(
  bases: (number | null)[],
  batter_id: number
): number | null {
  for (let i = 3; i >= 1; i--) {
...
function computeAtBatParticipants(
  atbat: ParsedAtBat,
  all_segments: ParsedSegment[]
): ParsedAtBatParticipant[] {
  ...
  for (const pid of participant_ids) {
    ...
    const had_hit_advance = player_segs.some(
    ...
    const had_walk = player_segs.some(
      s => s.reason === 'walk' || s.reason === 'walk_induced'
    );
    const got_out = player_segs.some(
      s => s.reason === 'hit_out' || s.reason === 'non_pitch_out'
    );
    const got_wounded = player_segs.some(s => s.reason === 'hit_wounded');
    
    // end_base: viimeisin segmentti tai start_base jos ei segmenttejä
    const end_base = player_segs.length > 0
      ? player_segs[player_segs.length - 1].to_base
      : start_base;
